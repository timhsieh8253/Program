...
/*------------------
  ...
  C.Wang 1204, 2015
 -------------------*/
void GamepadTest(GAMEPADid gpID, int butID, BOOL4 beP, float value)
{
   sprintf(tbDebugInfo, "%d %d %f", butID, beP, value);
}
...
/*------------------
  ...
  C.Wang 1204, 2015
 -------------------*/
void GamepadTest2(GAMEPADid gpID, int stickID, float x, float y, float sec)
{
   sprintf(tbDebugInfo, "%d %f %f %f", stickID, x, y, sec);
}
...
void FyMain(int argc, char *argv[])
{
   ...
   BOOL4 beReady = FyStartFlyWin32(caption, 0, 0, tbWorldWidth, tbWorldHeight, FALSE,
                                   TRUE, tbAALevel, FALSE, 0, NULL, TRUE);
   if (!beReady) {
   ...
   // check if we have gamepad connected
   int numGP;
   FyBeginGamepad(&numGP);
   if (numGP > 0) {
      // connect the gamepad to Fly2
      gpID = FyCreateGamepad();
      FnGamepad gp(gpID);
      gp.BindButtonFunction(FY_GP_1, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_2, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_3, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_4, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_5, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_6, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_7, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_8, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_9, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_10, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_LEFT_STICK, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_RIGHT_STICK, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_UP, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_RIGHT, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_DOWN, GamepadTest, FALSE);
      gp.BindButtonFunction(FY_GP_LEFT, GamepadTest, FALSE);
      gp.BindStickFunction(FY_GP_LEFT_STICK, GamepadTest2);
      gp.BindStickFunction(FY_GP_RIGHT_STICK, GamepadTest2);
   }
